...
var tempPath = path + ".watchdog.tmp";
...
using (var assembly = AssemblyDefinition.ReadAssembly(path, new ReaderParameters { ReadWrite = false }))
{
    ...
    il.Append(Instruction.Create(OpCodes.Brtrue, checkEye));
    ...
    il.Append(Instruction.Create(OpCodes.Brfalse, checkLip));
    ...
    il.Append(Instruction.Create(OpCodes.Brtrue, checkLip));
    ...
    il.Append(Instruction.Create(OpCodes.Brtrue, restart));
    ...
    {
        var processor = statusCallback.Body.GetILProcessor();
        var flagStore = statusCallback.Body.Instructions.FirstOrDefault(i =>
...
var verifyType = verify.MainModule.Types.Single(t => t.FullName == targetTypeName);
var verifyWatchdog = verifyType.Methods.SingleOrDefault(m => m.Name == "CheckFaceTrackingWatchdog");
...
var verifyUpdate = verifyType.Methods.Single(m => m.Name == "Update" && !m.HasParameters);
if (!verifyUpdate.Body.Instructions.Any(i =>
    (i.OpCode == OpCodes.Call || i.OpCode == OpCodes.Callvirt) &&
    i.Operand is MethodReference m &&
    m.Name == "CheckFaceTrackingWatchdog"))
{
    throw new InvalidOperationException("Update() does not call the watchdog.");
}
